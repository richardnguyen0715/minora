/** Knowledge list view */
let knState = { page:1, type:"", status:"", search:"", sort_by:"updated_at", sort_order:"desc" };
...
async function renderKnowledge() {
  ...
  mc.innerHTML = `
    ...
    <div class="toolbar">
      <div class="search-box"><span class="search-icon">🔍</span><input id="kn-search" placeholder="Search nodes..." value="${escapeHtml(knState.search)}" onkeyup="knDebounceSearch(this.value)"></div>
  ...
  try {
    const data = await API.listNodes({type:knState.type,status:knState.status,search:knState.search,sort_by:knState.sort_by,sort_order:knState.sort_order,page:knState.page,per_page:20});
    const area = document.getElementById("kn-table-area");
    if (!data.items.length) { area.innerHTML='<div class="empty-state"><div class="icon">📭</div><p>No nodes found</p></div>'; return; }
    const sortIcon = (col) => knState.sort_by===col ? (knState.sort_order==="asc"?"↑":"↓") : "";
    const sortCls = (col) => knState.sort_by===col ? "sorted" : "";
    let html = `<div class="card" style="padding:0;overflow:hidden"><table class="data-table"><thead><tr>
    ...
    for (const n of data.items) {
      const tags = (n.tags||[]).slice(0,3).map(t=>`<span class="tag tag-default">${escapeHtml(t)}</span>`).join("");
      html += `<tr>
...
function knDebounceSearch(val) { clearTimeout(knSearchTimer); knSearchTimer=setTimeout(()=>{knState.search=val;knState.page=1;renderKnowledge();},400); }
...
function openCreateNodeModal() {
  ...
  const footer = `<button class="btn btn-secondary" onclick="closeModal()">Cancel</button><button class="btn btn-primary" onclick="submitCreateNode()">Create</button>`;
  openModal("Create Knowledge Node", body, footer);
}

async function submitCreateNode() {
  try {
    const data = { id:document.getElementById("cf-id").value.trim(), type:document.getElementById("cf-type").value, title:document.getElementById("cf-title").value.trim(),
      status:document.getElementById("cf-status").value, content:document.getElementById("cf-content").value,
      tags:document.getElementById("cf-tags").value.split(",").map(s=>s.trim()).filter(Boolean) };
...
async function openEditNodeModal(nodeId) {
  try {
    ...
    const footer = `<button class="btn btn-secondary" onclick="closeModal()">Cancel</button><button class="btn btn-primary" onclick="submitEditNode('${escapeHtml(nodeId)}')">Save</button>`;
...
async function submitEditNode(nodeId) {
  try {
    const data = { title:document.getElementById("ef-title").value.trim(), status:document.getElementById("ef-status").value,
    ...
    const conf = document.getElementById("ef-conf").value;
    if(conf!=="") data.confidence = parseFloat(conf);
